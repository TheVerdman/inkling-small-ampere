...
DISPLAY_NAME="inkling-w8a16-layer-tp4-${RUN_TIMESTAMP}"
ARTIFACT_OBJECT="inkling-small-ampere/w8a16-layer/${DISPLAY_NAME}.json"
ARTIFACT_URI="${BUCKET}/${ARTIFACT_OBJECT}"
TEMP_DIR="$(mktemp -d)"
CONFIG_PATH="${TEMP_DIR}/vertex-w8a16-inkling-layer.yaml"
LOCAL_RESULT="results/raw/${DISPLAY_NAME}.json"
PROBE_SOURCE="scripts/gpu/w8a16_inkling_moe_layer_probe.py"
...
case "${ACCELERATOR_COUNT}" in
  4) ;;
  *)
    echo "This layer contract requires exactly four accelerators." >&2
    exit 2
    ;;
esac
...
- machineSpec:
  ...
  containerSpec:
    ...
    - |
      ...
      echo '${PROBE_BASE64}' | base64 --decode > /tmp/w8a16_inkling_moe_layer_probe.py
      ...
      python3 /tmp/apply_unified_diff.py \
        ...
        --include-prefix vllm/
      torchrun --standalone --nproc-per-node=4 /tmp/w8a16_inkling_moe_layer_probe.py
    env:
    - name: PROBE_ID
      value: ${DISPLAY_NAME}
