...
DISPLAY_NAME="inkling-flex-attn-a100-${RUN_TIMESTAMP}"
ARTIFACT_OBJECT="inkling-small-ampere/flex-attention/${DISPLAY_NAME}.json"
ARTIFACT_URI="${BUCKET}/${ARTIFACT_OBJECT}"
TEMP_DIR="$(mktemp -d)"
CONFIG_PATH="${TEMP_DIR}/vertex-flex-attention.yaml"
LOCAL_RESULT="results/raw/${DISPLAY_NAME}.json"
PROBE_SOURCE="scripts/gpu/flex_relative_attention_probe.py"
...
- machineSpec:
  ...
  containerSpec:
    ...
    args:
    - |
      echo '${PROBE_BASE64}' | base64 --decode > /tmp/flex_relative_attention_probe.py
      python3 /tmp/flex_relative_attention_probe.py
    env:
    - name: PROBE_ID
      value: ${DISPLAY_NAME}
