...
DISPLAY_NAME="w8a16-linear-tp4-${RUN_TIMESTAMP}"
ARTIFACT_OBJECT="inkling-small-ampere/w8a16-linear/${DISPLAY_NAME}.json"
ARTIFACT_URI="${BUCKET}/${ARTIFACT_OBJECT}"
TEMP_DIR="$(mktemp -d)"
CONFIG_PATH="${TEMP_DIR}/vertex-w8a16-linear.yaml"
LOCAL_RESULT="results/raw/${DISPLAY_NAME}.json"
PROBE_SOURCE="scripts/gpu/w8a16_linear_tp4_probe.py"
...
case "${ACCELERATOR_COUNT}" in
  4) ;;
  *)
    echo "This linear contract requires exactly four accelerators." >&2
    exit 2
    ;;
esac
...
- machineSpec:
  ...
  containerSpec:
    ...
    args:
    - |
      echo '${PROBE_BASE64}' | base64 --decode > /tmp/w8a16_linear_tp4_probe.py
      torchrun --standalone --nproc-per-node=4 /tmp/w8a16_linear_tp4_probe.py
    env:
    - name: PROBE_ID
      value: ${DISPLAY_NAME}
